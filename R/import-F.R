...
# Determina família
get_F_or_I_family_from_xpert_data <- function(novos_itens){

  novos_itens <-
    novos_itens |>
    mutate(fam_mat = case_when(
      str_detect(prj_sigla, "CX") ~   "UNKN",
      str_detect(prj_sigla, "X70") ~  "X70",
      str_detect(prj_sigla, "X62") ~  "X70",
      str_detect(prj_sigla, "1312") ~ "1312",
      str_detect(prj_sigla, "VIVA") ~ "VIVA",
      str_detect(prj_sigla, "V230") ~ "GOLF",
      str_detect(prj_sigla, "UPPP") ~ "UP",
      str_detect(prj_sigla, "UPC") ~  "UP",
      str_detect(prj_sigla, "UP") ~   "UP",
      str_detect(prj_sigla, "U79P") ~ "U79",
      str_detect(prj_sigla, "SCXD") ~ "SC",
      str_detect(prj_sigla, "SCDI") ~ "SC",
      str_detect(prj_sigla, "PM7") ~  "PM7",
      str_detect(prj_sigla, "PA") ~   "PA",
      str_detect(prj_sigla, "P426") ~ "426",
      str_detect(prj_sigla, "OTH") ~  "UNKN",
      str_detect(prj_sigla, "NTC") ~  "NTC",
      str_detect(prj_sigla, "NCG") ~  "NCG",
      str_detect(prj_sigla, "MQBF") ~ "MQB",
      str_detect(prj_sigla, "MQB") ~  "MQB",
      str_detect(prj_sigla, "LTC") ~  "LTC",
      str_detect(prj_sigla, "L98") ~  "L98",
      str_detect(prj_sigla, "HPN") ~  "HPN",
      str_detect(prj_sigla, "HJF") ~  "HJF",
      str_detect(prj_sigla, "HJDE") ~ "HJDE",
      str_detect(prj_sigla, "HJDD") ~ "HJDD",
      str_detect(prj_sigla, "FX/J") ~ "FOX",
      str_detect(prj_sigla, "FPN") ~  "FPN",
      str_detect(prj_sigla, "FOX") ~  "FOX",
      str_detect(prj_sigla, "F1H") ~  "F1H",
      str_detect(prj_sigla, "CUVF") ~ "246",
      str_detect(prj_sigla, "CUV") ~  "246",
      str_detect(prj_sigla, "C4P") ~  "C4",
      str_detect(prj_sigla, "C4E") ~  "C4",
      str_detect(prj_sigla, "B02A") ~ "B02A",
      str_detect(prj_sigla, "AI94") ~ "I94",
      str_detect(prj_sigla, "AI58") ~ "I58",
      str_detect(prj_sigla, "A91") ~  "208",
      str_detect(prj_sigla, "516T") ~ "516",
      str_detect(prj_sigla, "270H") ~ "270",
      str_detect(prj_sigla, "23X") ~  "23X",
      str_detect(prj_sigla, "220P") ~ "220",
      str_detect(prj_sigla, "220C") ~ "220",
      str_detect(prj_sigla, "216T") ~ "216",
      str_detect(prj_sigla, "216C") ~ "216",
      str_detect(prj_sigla, "216U") ~ "216",
      str_detect(prj_sigla, "A04")  ~ "A04",
      TRUE ~ "UNKN"
    )) |>
    mutate(fam_com = case_when(
      str_detect(prj_desc, "Renault") ~ "RN",
      str_detect(prj_desc, "Logan")   ~ "RN",
      str_detect(prj_desc, "RSA")     ~ "RN",
      str_detect(prj_desc, "Volkswagen") ~ "VW",
      str_detect(prj_desc, "Audi")    ~ "VW",
      str_detect(prj_desc, "Nissan")  ~ "UNKN",
      str_detect(prj_desc, "Duster")  ~ "RN",
      str_detect(prj_desc, "VW")      ~ "VW",
      str_detect(prj_desc, "PSA") ~ "STEL",
      str_detect(prj_desc, "MQB") ~ "VW",
      str_detect(prj_desc, "Chevrolet") ~ "GMB",
      str_detect(prj_desc, "Peugeot") ~ "STEL",
      str_detect(prj_desc, "GMB") ~ "GMB",
      str_detect(prj_desc, "L98") ~ "MBB",
      str_detect(prj_desc, "FPN") ~ "FPN",
      str_detect(prj_desc, "PA") ~ "MBB",
      str_detect(prj_desc, "Stellantis") ~ "STEL",
      str_detect(prj_desc, "Scania") ~ "SCANIA",
      TRUE ~ "UNKN"
    )) |>
    mutate(grupo_de_estoque = ifelse(str_detect(item, "^F"), "10", "20")) |>
    mutate(fam_mat = str_c(str_sub(fam_com, 1, 2),
                           fam_mat,
                           grupo_de_estoque)) |>
    mutate(fam_mat = ifelse(str_detect(fam_mat, "UNKN"), "UNKN", fam_mat))
}
