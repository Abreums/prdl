...
f_simoldes <- 
  ...
  select(cod_comp, item, estabelecimento, desc, desc2,prj_sigla, prj_desc)
f_simoldes |> write_rds("data/f-simoldes.rds")
f_simoldes <- read_rds("data/f-simoldes.rds")

f_simoldes |> write_csv2("codigos-f.csv")
...
f_spi <- 
  f_spi |> 
  left_join(itens |> filter(estabelecimento == "SPI")) 

f_spb <- 
  f_spb |> 
  left_join(itens |> filter(estabelecimento == "SPB"))
